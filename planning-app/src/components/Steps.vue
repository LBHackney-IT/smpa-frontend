    ...
    <div class="steps govuk-body">
			<div v-for="(step, index) in steps" v-bind:key="index" class="steps__single-step">
				<div class="steps__step-guide">
					...
					<div class="steps__line-guide" v-bind:class="[(currentRoute === step.name || step.isActive) ? 'steps--current-step' : '']"></div>
					<div v-if="index == steps.length - 1" class="steps__line-end"></div>
				</div>
				
				<div class="steps__step-info">
					<h4>{{step.title}}</h4>
					<ShowHide :active="step.isActive" v-bind:content="step.content"/>
				</div>
			</div>
    </div>
</template>
